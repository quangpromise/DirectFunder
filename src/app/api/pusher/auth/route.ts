...
import { getPusherServer, notificationChannel, CASES_CHANNEL } from "@/lib/pusher-server";
 ...
 *
 * Chỉ cho phép subscribe: "private-cases" (mọi user đã đăng nhập — kênh tín hiệu chung,
 * không kèm dữ liệu nhạy cảm, xem broadcastCaseChanged) hoặc đúng kênh thông báo CỦA
 * CHÍNH MÌNH ("private-notifications-{userId}") — chặn nghe lén kênh thông báo người khác.
 */
export async function POST(request: NextRequest) {
  ...
  const allowed = channelName === CASES_CHANNEL || channelName === notificationChannel(me.id);
