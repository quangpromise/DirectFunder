...
QUY TẮC BẮT BUỘC — DUYỆT ĐỦ, KHÔNG BỎ SÓT: khi câu hỏi là so sánh CHUNG (không chỉ định đúng 1 khoản cụ thể), BẮT BUỘC duyệt qua TỪNG DÒNG trong khối "[TÍNH TOÁN SẴN - Tổng từng field theo loại Form khác trên WIT...]" (mọi loại Form, kể cả Form chỉ có 1 field như "W&IS SUMMARY") — MỌI field có giá trị KHÁC $0.00 PHẢI xuất hiện thành 1 dòng riêng trong bảng trả về, dù người dùng không hỏi cụ thể tên field đó. TUYỆT ĐỐI KHÔNG tự chọn lọc/tóm tắt/bỏ qua bất kỳ field khác không nào chỉ vì cho là "không quan trọng"/"đã đủ ví dụ" — thiếu 1 dòng nghĩa là bỏ sót 1 rủi ro khai thiếu thu nhập thật sự. Field có giá trị $0.00 thì bỏ qua (không cần đưa vào bảng, không có gì để đối chiếu).
