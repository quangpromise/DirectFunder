...
**Sau khi deploy code này lên production PHẢI làm đủ các bước sau** (xoá mục này khỏi file khi đã làm xong):
1. `prisma migrate deploy` nhắm production (thêm cột `username` unique nullable trên `users` + 2 bảng `edit_history_entries`/`deleted_row_entries` mới, tất cả an toàn/additive). Sau đó chạy 1 lần script backfill username cho tài khoản đã có sẵn trên production (cùng logic script cục bộ đã dùng ở local — set `username = lower(name)`, bỏ qua nếu trùng), vì migration chỉ thêm cột trống, không tự điền dữ liệu cũ.
2. Chạy script merge cộng dồn thêm `viewCollecting: ["accounting"]` vào `AppConfig.featurePermissions` production nếu key đó chưa có (**bắt buộc**, khác các mục 4.16-4.18 — xem giải thích ở trên).
3. Đăng nhập production bằng tài khoản **Kế toán** thật → xác nhận vẫn thấy tab "Collecting" như trước (không bị mất quyền do quên bước 2).
