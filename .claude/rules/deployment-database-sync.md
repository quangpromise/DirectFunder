...
**Sau khi deploy code này lên production PHẢI làm đủ các bước sau** (xoá mục này khỏi file khi đã làm xong):
1. `prisma migrate deploy` nhắm production (tạo bảng `cpa_review_records` mới VỚI cột `month` đã có sẵn từ đầu + thêm cột `cpaReviewSheetConfig` trên `app_config`, cả 2 an toàn/additive — production chưa có dữ liệu CPA Review nào nên không cần lo bước backfill `month` như đã làm ở DB dev).
...
7. Vào chính Google Sheet đó, share quyền **Editor** cho email Service Account (bước 3/5), rồi Extensions → Apps Script, dán đoạn script từ bước 6, Save, chọn hàm **`installCpaReviewTriggers`** ở dropdown (KHÔNG phải `onEdit`) rồi bấm Run 1 lần — vừa cấp quyền (`UrlFetchApp`/`PropertiesService`/`ScriptApp`, popup xin quyền, Admin bấm đồng ý) vừa cài trigger hẹn giờ quét Ghi chú (Note) mỗi 5 phút; `onEdit` là simple trigger nên tự chạy, không cần cài riêng.
...
15. Bấm icon 📌 cạnh ô "Ngày" 1 năm bất kỳ trong app, gõ 1 ghi chú → mở đúng ô "Ngày" tương ứng trên Sheet, hover vào góc đỏ ô → xác nhận thấy đúng Note vừa gõ trong vài giây (App→Sheet tức thời). Ngược lại: chuột phải 1 ô "Ngày" khác trên Sheet → Insert note → gõ thử → đợi tối đa ~5 phút (không cần F5) → xác nhận app hiện đúng ghi chú đó (Sheet→App quét định kỳ, có độ trễ đã xác nhận với user).
...
**Sau khi deploy code này lên production PHẢI làm đủ các bước sau** (xoá mục này khỏi file khi đã làm xong):
1. `prisma migrate deploy` nhắm production (thêm 3 cột `fcDate`/`processingDate`/`elDate` trên `cases`, an toàn/additive, nullable).
...
**Sau khi deploy code này lên production PHẢI làm đủ các bước sau** (xoá mục này khỏi file khi đã làm xong):
1. `prisma migrate deploy` nhắm production (thêm cột `cpaReviewTestSentAt` trên `cases`, an toàn/additive, nullable).
